...
Open this repository with the included `.devcontainer/devcontainer.json`. It builds on plain `ubuntu:24.04` rather than a devcontainer base image — `install.sh` already provisions everything the container needs, so a heavier base (1GB+ vs. 141MB) would only add packages that get reinstalled or go unused. Ubuntu's official image already ships a uid 1000 user (`ubuntu`, for cloud-init); the image renames it to `dev` with passwordless sudo, matching the bind-mounted workspace's usual ownership on Linux hosts. The post-create command runs the same bootstrap as that user, so the configuration is linked in the container user's home.
