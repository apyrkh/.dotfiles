...
Ubuntu, runs `scripts/install-mac.sh` or `scripts/install-linux.sh`, then
`scripts/symlinks.sh`. `install-work.sh` runs `install.sh` plus macOS work GUI
apps/Docker; `install-home.sh` runs `install-work.sh` plus personal apps. The
...
- `scripts/` — `install-mac.sh`/`install-linux.sh` (package installers) and `symlinks.sh` (idempotent linking)
...
- **Neovim-only tooling** (LSP servers, `conform.nvim` formatters like `prettierd`/`eslint_d`/`pgformatter`) is installed by Mason from inside `home/.config/nvim/lua/plugins/code.lua`, not by `scripts/install-mac.sh`/`scripts/install-linux.sh` — it's cross-platform already and shouldn't be duplicated per-OS. `mason-tool-installer`'s own `VimEnter` auto-install races the plugin's own lazy-load, so `code.lua` calls `check_install()` itself right after `setup()`.
- **Tiers:** `install.sh` is the base CLI tier (any machine, incl. Ubuntu/Dev Container via `scripts/install-linux.sh`). `install-work.sh` runs `install.sh` then installs macOS work GUI apps and Docker/Colima via plain `brew install`. `install-home.sh` runs `install-work.sh` then installs personal media/gaming apps, also via `brew install`. No Brewfile — package names live directly in each script as flat arrays, grouped with `# === category ===` comments.
