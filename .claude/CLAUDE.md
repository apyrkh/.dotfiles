...
`scripts/symlinks.sh`. `install-mac-work.sh` runs `install.sh` plus macOS work GUI
...
- `scripts/` — `install-mac.sh`/`install-ubuntu.sh` (package installers) and `symlinks.sh` (idempotent linking)
